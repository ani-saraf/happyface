...
DYNAMO_TBL_FACEGROUPS = 'FaceGroups'
...
def process_image(event=None, context=None):
    ...
    tmp_path = os.path.join(tmp, os.path.basename(key).rstrip("/").replace("/", "\\"))
    try:
    ...
    for index, image in enumerate(response['FaceDetails']):
        path = os.path.join(tmp, os.path.basename(key).rstrip("/"))
        img = Image.open(path)
        ...
        dir_path = os.path.join(tmp, os.path.basename(key).split(".")[0])
        ...
        s3_thumbnail_path = "thumbnail/{}".format(
            os.path.basename(key).split(".")[0] + "_" + os.path.basename(thumbnail_path))
...
def download_files(key):
    try:
        os.mkdir(os.path.join(tmp, GROUP_PHOTOS), mode=0o777)
    except Exception as e:
        logger.error("Exception occurred while creating group_photos dir in tmp: {}".format(str(e)))
    path = os.path.join(tmp, os.path.basename(key).rstrip("/"))
    try:
        s3.meta.client.download_file(s3_bucket, key, path)
    except Exception as e:
        logger.error('Error occurred while downloading image file from s3: {}'.format(str(e)))
        raise Exception('Error occurred while downloading image file from s3: ', e)
    size = (x, y)
...
def list_s3_objects():
    ...
    bucket_prefix = GROUP_PHOTOS + "/"
    try:
        objs = bucket.objects.filter(Prefix=bucket_prefix)
    except Exception as e:
        logger.error('Error occurred while listing s3 objects: {}'.format(str(e)))
        raise Exception('Error occurred while listing s3 objects: ', e)
